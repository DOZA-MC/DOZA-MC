<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=6A5ACD&height=150&section=header&text=DOZA%20Server&fontSize=70&fontColor=FFFFFF&animation=fadeIn" />
  <br>
  <h2>🌙 DQZA</h2>
  <p>Welcome to the DOZA Server!<br>Enjoy your stay!</p>
  <br>
  <a href="https://discord.gg/cpmeZAACvg" target="_blank">
    <img src="https://img.shields.io/badge/Join_DOZA-5865F2?style=for-the-badge&logo=discord&logoColor=white" alt="Join DOZA" />
  </a>
</div>

---

<div align="center">

### 📜 Verification Fine Print
By submitting an application in the Discord server before or after the rules were established, you are required to read and follow the rules. If a moderated action is taken against you, <b>DO NOT claim you didn't read the rules.</b> You will be dealt with accordingly. These rules are subject to change, and you are obligated to check them periodically. Ignorance of an updated rule is not an excuse.

### 📢 Support Notice
Need help appealing, reporting, or asking a question? Please direct all inquiries to our dedicated <b>support channel</b>.

### 🚫 Warning System (Discord Only)
You will receive a maximum of <b>3 warnings</b> before being automatically banned from the Discord server. Please ensure to follow our rules and guidelines.

### 👑 Leadership
<img src="https://img.shields.io/badge/Owner_DOZA-FFD700?style=for-the-badge&logo=discord&logoColor=000000" /><br>
<code>786081192926773248</code> ｜ <b>xdqza</b>
<br><br>
<img src="https://img.shields.io/badge/Architect_Astral-6A5ACD?style=for-the-badge&logo=discord&logoColor=FFFFFF" /><br>
<code>681045492704215064</code> ｜ <b>lcul</b>

</div>

---

<div align="center">
  <i>You can access the rules by clicking one of the dropdowns below.</i>
</div>

<br>

<details>
  <summary><b>📜 Discord Server Rules & Guidelines</b></summary>
  <br>

  These rules have been modified on 7.8.26 | 5:30PM EST | By Astral

  Note: For each rule/subrules, I put the Tiers have what type of punishment is received if that rule/subrule is broken.
  
  <br>

  ### 1. Mandatory Verification
  ▸ **Screening Process:** All members must complete a mandatory screening process via our verification system to gain access to the rest of the server.<br>
  ▸ **No Alternatives:** There are absolutely no alternative methods for bypassing this step.<br>
  ▸ **Security:** This measure is strictly enforced to maintain high security standards and ensure the server remains a safe space for everyone.
  ▸ **Failure to compliy:** Will result in being tempbanned for 1 month

  <br>

  ### 2. Respect Everyone (Toxicity & Harassment)
  T2 Treat others with kindness and respect. Racism, sexism, homophobia, harassment, hate speech, and general toxicity are strictly prohibited.

  <br>

  ### 3. No NSFW or Inappropriate Content
  T4 This server is for general audiences. NSFW, explicit, gore, or sexually suggestive content (including text and images) is banned.

  <br>

  ### 4. Compliance, Privacy, and Communication
  ▸ T3 **Platform Compliance:** All members must adhere to [Discord's Terms of Service](https://discord.com/terms) and [Privacy Guidelines](https://discord.com/privacy). Any activity that violates Discord's policies is prohibited here.<br>
  ▸ **Language:** Please speak English in public channels to ensure clear communication and prevent misunderstandings.<br>
  ▸ T2 **Standards:** Maintain a mature standard of communication; avoid excessive "brainrot" slang or inappropriate roleplay terms.

  <br>

  ### 5. Privacy and Safety
  ▸ T3 **Do not** share personal information (yours or others), such as addresses, passwords, or real names.<br>
  ▸ T1 **Do not** share private messages or DMs without explicit consent from all parties involved.<br>
  ▸ T4 **Do not** engage in illegal activities or share pirated content.

  <br>

  ### 6. Spam and Advertising
  ▸ T1 Do not spam messages, links, emojis, or mentions (including mass `@here`/`@everyone` pings).<br>
  ▸ T2 Do not advertise external servers, products, or services without prior permission from the Administration Team.

  <br>

  ### 7. Server Structure & Staff
  ▸ T1 **Channels:** Use channels for their intended purpose (e.g., keep discussions relevant to the channel topic).<br>
  ▸ T2 **Special Channels:** Do not misuse the special channels, Punishment is a warn.
  ▸ **Staff Instructions:** Follow the instructions of the staff team. They are responsible for maintaining order.<br>
  ▸ T4 **Bots:** Do not use unauthorized bots, user-bots, or automation tools.

  <br>

  ### 8. Role Requests
  T2 Do not ask for ranks, admin positions, or special roles. Roles are assigned based on merit, effort, and necessity, not upon request.

  <br>

  ### 9. Artist Role & AI Art Policy
  ▸ T1 **Obtaining the Role:** To obtain the **Artist** role, you must send **original art** in the `#media` channel.<br>
  ▸ T1 **Patience:** Do not ask any staff member about the status of your application. Staff will review submissions in their own time.<br>
  ▸ **Inquiries:** If you have posted original art and have not received the role after **3 days**, you may politely ask a staff member about it.<br>
  ▸ T2 **AI Art Policy:** Posting AI-generated content is prohibited, as it is not original work. Doing so will result in a **Warning**. If you hold the Artist role and are found posting AI-generated content, your role will be **revoked immediately**, and you will be **permanently ineligible** to obtain the role again.

  <br>

  ### 10. Account Conduct
  ▸ T2 **Identity:** Your username and avatar must be appropriate for a general audience.<br>
  ▸ T4 **Impersonation:** Impersonation of staff, other members, or public figures is banned.<br>
  ▸ T2 **Loopholes:** Do not attempt to bypass rules using technicalities or loopholes. The spirit of the rules is as important as their literal interpretation.

  <br>

  ### 11. Moderation & Appeals
  ▸ If you have a question regarding a moderation action (mute, ban, etc.) or an application, **open a ticket** in the designated channel.<br>
  ▸ Do not discuss bans, mutes, or application statuses in public chat or DM staff members about them.
  

  <br>

  ---
  ### Discord Punishment Guidelines
  Failure to follow the rules will result in consequences based on the severity of the offense and the user's history.

  **Tier 1: Minor Infractions** *(Accidental rule breaks, minor spam, slight disrespect)*<br>
  Punishment: `Verbal Warning` or `Timeout (up to 10 minutes)`

  **Tier 2: Moderate Infractions** *(Intentional disruption, repeated spamming, mild toxicity)*<br>
  Punishment: `Formal Warning` or `Timeout (1 hour)`

  **Tier 3: Severe Infractions** *(Harassment, serious toxicity, unauthorized advertising, posting AI content)*<br>
  Punishment: `Softban` (Kick and message delete) or `Timed Ban (1-7 days)`

  **Tier 4: Critical Infractions** *(Illegal activities, doxxing, severe hate speech, NSFW content)*<br>
  Punishment: `Permanent Ban`

  <br>

  **Repeated Offenses & Escalation**
  ▸ **1st Offense:** Warning / Timeout<br>
  ▸ **2nd Offense:** Severe Warning / Temporary Ban<br>
  ▸ **3rd Offense:** Permanent Ban<br>
  <i>*Note: Staff reserve the right to skip tiers based on the severity of the situation.*</i>

</details>

<br>

<details>
  <summary><b>⛏️ Crossplay Server Rules (Members)</b></summary>
  <br>

  <i>All Minecraft Server Members (Not for discord). Staff Members don't follow the following rules (1, 5, 6).</i>

  <br>

  ### 1: 🗣️ Be Respectful and ENGLISH ONLY
  Please be respectful with one and other. This is a GOLDEN Rule. And please make sure to speak in English; any other languages aren't welcomed.

  <br>

  ### 2: 🔨 No Client Modification or Using Exploits
  This includes finding glitches and abusing them, doing stuff that would give you a higher advantage compared to others (Also includes purposely glitching to somewhere).
  This also includes any texture packs that give players a higher and cheesey advantages. Players can ask either of the owners if they are allowed to use certian packs.

  <br>

  ### 3: ⚠️ 🔨 No Bullying and saying any slurs of ANY KIND
  It's as simple as it gets: just don't bully or say any slurs.

  <br>

  ### 4: Spawn's Rules
  ▸ ⚠️ 🔨 No Spawnkilling: When a user spawns at spawn, give them a grace period of time of 3 minutes.<br>
  ▸ 🔨 No Blowing up spawn or griefing it.<br>
  ▸ ⚠️ Do not make block towers.<br>
  ▸ ⚠️ Do not destroy any creative structures that look nice.

  <br>

  ### 5: Treat Staff Right
  ▸ 🗣️ ⚠️ No matter what's the matter, always treat anyone who is a staff member with upmost highest respect. If you don't, it may be used against you.<br>
  ▸ ⚠️ Do not ask any staff member about any case that has been taken against a user in the Crossplay Server.

  <br>

  ### 6: 💫 No Paradoxes or Manipulation
  This is basically saying don't try and outsmart the staff in any way or use loopholes. If you do, that will fall under this rule. Depending on the action you did, it may get rerouted to another rule and that action applied to you.

  <br>

  ### 7: ⚠️ No Griefing (within 5k Blocks of spawn)
  This means no griefing anywhere unless given permission (Must save permission as proof or you will be considered breaking this). No using respawn anchors, anything TNT, and anything explosive to destroy anywhere relating to demolishing structures or manually breaking structures.

  <br>

  ### 8: ⚠️ Do not ask staff for ANY item or any advantage
  This means don't beg or ask any staff representative in the MC Server for any item or advantage. This also includes asking for any roles/prefixes. This also falls under trying to trade with staff members.

  <br>

  ### 9: ⚠️ Please follow any instruction given by a staff member
  You are obligated to follow any instruction given by a staff member and failure to comply will result in a warning.

  <br>

  ### 10: ⚠️ Do not complain about any of the rules
  This means no complaining about any rules and don't try to make suggestions or ask staff directly about these rules why they were implemented.

  <br>

  ---
  ### Appealing Actions Taken Against You
  If support is closed, please be patient and wait.<br>
  You are able to appeal for an action taken against you as long as you have **physical PROOF** proving that it is appeal-worthy.

  <br>

  ### Punishment List
  If a rule has a symbol next to the number, that's the punishment that will fall on you if broken. If there are multiple, that will be the next action taken if broken again.<br>
  If you have more than 6 warns, you will be banned from the MC Crossplay Server.

  ▸ 🗣️ (Verbal Warning)<br>
  ▸ ⚠️ (Warning)<br>
  ▸ 🔨 (Banned)

  <br>
  <i>Staff Team sends their regards!</i>

</details>

<br>

<details>
  <summary><b>🛡️ Crossplay Staff Rules</b></summary>
  <br>

  <i>All Minecraft Staff Team Members (Only applied on the actual MC Server and not discord).</i><br>
  <i>These rules will be applied by hierarchy. The Owners may disregard these rules.</i>

  <br>

  ### 1: 🗣️ Be Respectful
  Be respectful to all members of the Crossplay Server unless they treat you in a wrongly manner.

  <br>

  ### 2: ⚖️ No Abusing Powers
  Do not abuse any powers for fun or in any matter in any way. This also means only follow the guide of what action to take according to the Crossplay Server Rules for the members and don't act against any of those rules.

  <br>

  ### 3: 🗣️ Treat Other Staff Right
  Treat other staff members kindly and not hostile to each-other.

  <br>

  ### 4: 🗣️ Follow all normal Crossplay Rules
  With the exception of the excluded rules mentioned in the Member Rules (1, 5, 6).

  <br>

  ### 5: 🗣️ Safety Identity
  Do not mention to anyone who isn't a staff member about any case that has happened within the Server.

  <br>

  ---
  ### Punishment for Staff
  If a rule has a symbol next to the number, that's the punishment that will fall on you if broken. If there are multiple, that will be the next action taken if broken again.

  ▸ 🗣️ (Verbal Notice)<br>
  ▸ ⚖️ (Judgement from higher-ups)

  <br>
  <i>Staff Team sends their regards!</i>

</details>

<br>

<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=6A5ACD&height=100&section=footer" />
</div>
